...
const outputDir = "temp_shadow";
const finalOutputDir = "stanzas_shadow";
...
stanzas.forEach((stanza, stanzaIndex) => {
  ...
  for (let i = 0; i <= stanza.length - 1; i++) {
    ...
    const shadowWav = `${outputDir}/stanza${stanzaIndex}_shadow${i}.wav`;
    ...
    // Generate shadow of same duration as WAV
    execSync(
      `ffmpeg -loglevel quiet -y -i "${audioFile}" -ss ${start} -t ${duration} -acodec pcm_s16le -ar 44100 -ac 1 -filter:a "volume=0.2" "${shadowWav}"`
    );

    // Concatenate line + shadow as WAV
    const listFile = `${outputDir}/concat_combo_${stanzaIndex}_${i}.txt`;
    fs.writeFileSync(
      listFile,
      [
        `file '${path.resolve(lineWav)}'`,
        `file '${path.resolve(shadowWav)}'`,
      ].join("\n")
    );
  ...
  const lyrics = stanza.map((line) => line.text).join("\n");
  execSync(
    `ffmpeg -i "${finalMp3}" -metadata artist="Isha" -metadata album="Guru Pooja Memorization" -metadata title="${firstLineText}" -metadata track="${
      stanzaIndex + 1
  ...
  // execSync(
  //   `id3v2 --USLT "eng::${lyrics}" -a "Guru Pooja Call & Response with Shadow" -A "Isha" -t "${firstLineText}" -T ${
  //     stanzaIndex + 1
  //   } "${finalMp3}"`
  // );
